    ...
    while (!file_string_stream.eof()) {
        ...
        if (line_header[0] == 'v' && line_header[1] == 'n') {
            ...
            int matches = sscanf(line_header, "vn %f %f %f\n", &normal[0], &normal[1],
                                 &normal[2]);
            if (matches != 3) {
                __android_log_print(ANDROID_LOG_VERBOSE, "TeamPraktikum","Format of 'vn float float float' required for each normal line");
                return false;
            }
            ...
            if (matches != 2) {
                __android_log_print(ANDROID_LOG_VERBOSE, "TeamPraktikum","Format of 'vt float float' required for each texture uv line");
                return false;
            }
            ...
            int matches = sscanf(line_header, "v %f %f %f\n", &vertex[0], &vertex[1],
                                 &vertex[2]);
            if (matches != 3) {
                __android_log_print(ANDROID_LOG_VERBOSE, "TeamPraktikum","Format of 'v float float float' required for each vertice line");
                return false;
            }
...
//                                    "each face");
    ...
    if (is_normal_available && normal_indices.size() != vertex_indices.size()) {
        __android_log_print(ANDROID_LOG_VERBOSE, "TeamPraktikum","Obj normal indices does not equal to vertex indices.");
        return false;
    }

    if (is_uv_available && uv_indices.size() != vertex_indices.size()) {
        __android_log_print(ANDROID_LOG_VERBOSE, "TeamPraktikum","Obj UV indices does not equal to vertex indices.");
        return false;
    }
...
bool LoadTextFileFromAssetManager(const char* file_name,
    ...
    if (asset == nullptr) {
        __android_log_print(ANDROID_LOG_VERBOSE, "TeamPraktikum","Error opening asset %s", file_name);
        return false;
    }
    ...
    if (ret <= 0) {
        __android_log_print(ANDROID_LOG_VERBOSE, "TeamPraktikum","Failed to open file: %s", file_name);
        AAsset_close(asset);
        return false;
    }
